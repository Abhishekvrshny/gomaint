...
func setupApp() (*App, error) {
	...
	consumerGroupName := getEnv("KAFKA_CONSUMER_GROUP", "gomaint-consumer-group")
	...
	// Create Sarama consumer group using the builder
	consumerGroup, err := adapters.NewSaramaConsumerGroupBuilder(brokers, consumerGroupName).
		WithOffsetInitial(sarama.OffsetOldest). // Start from oldest unread messages
	...
	// Create Sarama adapter
	consumer := adapters.NewSaramaAdapter(consumerGroup, topics, messageProcessor)
	...
	// Create generic Kafka handler with Sarama adapter
	...
	app := &App{
		...
		topics:        topics,
		server: &http.Server{
			Addr:    ":8083", // Different port than other services
			Handler: mux,
		},
	}
